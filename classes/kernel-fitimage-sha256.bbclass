...
fitimage_assemble() {
	...
	if [ "x${UBOOT_SIGN_ENABLE}" = "x1" ] ; then
		echo "u-boot.dtb binary before: "
		openssl sha1 "${DEPLOY_DIR_IMAGE}/${UBOOT_DTB_BINARY}"
		uboot-mkimage \
			${@'-D "${UBOOT_MKIMAGE_DTCOPTS}"' if len('${UBOOT_MKIMAGE_DTCOPTS}') else ''} \
			-F -k "${UBOOT_SIGN_KEYDIR}" \
			-K "${DEPLOY_DIR_IMAGE}/${UBOOT_DTB_BINARY}" \
			-r arch/${ARCH}/boot/${2}
		echo "u-boot.dtb binary after:"
		openssl sha1 "${DEPLOY_DIR_IMAGE}/${UBOOT_DTB_BINARY}"
	fi
}

do_assemble_fitimage() {
	cert_file_name=${UBOOT_SIGN_KEYDIR}/${UBOOT_SIGN_KEYNAME}.cert
	crt_file_name=${UBOOT_SIGN_KEYDIR}/${UBOOT_SIGN_KEYNAME}.crt
	if [[ ! -f "${crt_file_name}" && -f "${cert_file_name}" ]] ; then
		cp ${cert_file_name} ${crt_file_name}
	fi
